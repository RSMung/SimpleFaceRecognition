...
def load_resnet18_elasticfaceplusarc_ckp(cls_model:nn.Module, elasticfaceplusarc_loss_func, dataset_name, model_name, ckp_time_stamp):
    ckp_path = os.path.join(
...
def defineModel(
    ...
    if backbone_type == "resnet18" and loss_fuc_type == "ElasticFacePlus-Arc":
        cls_model = Resnet18()
        elasticfaceplusarc_loss_func = ElasticArcFace(
            in_features=cls_model.feats_dim, out_features=n_class, plus=True)

        # ckp_time_stamp 不为空时加载预训练权重
        if ckp_time_stamp is not None:
            model_name = backbone_type + "_" + loss_fuc_type
            cls_model, elasticfaceplusarc_loss_func = load_resnet18_elasticfaceplusarc_ckp(
                cls_model, elasticfaceplusarc_loss_func,
                dataset_name, model_name, ckp_time_stamp
            )

        return cls_model.cuda(), elasticfaceplusarc_loss_func.cuda()
