...
class TrainCosFaceParams(ParamsParent):
    ...
    # ckp_time_stamp = "2024-12-10_18-20"   # exp11   cifar100   resnet18   CosFace
    ckp_time_stamp = "2024-12-10_18-21"   # exp12   cifar10   resnet18   CosFace
    ...
    # nohup python -u main.py > ./CosFace/log/2024-12-10_18-20.txt 2>&1 &
    # exp11      1
    # nohup python -u main.py > ./CosFace/log/2024-12-10_18-21.txt 2>&1 &
    # exp12      1
...
def train_procedure(
        params:TrainCosFaceParams,
        cls_model:nn.Module, cosFace_loss_func:nn.Module,
        train_dataloader,
        val_dataloader,
        # test_dataloader
    ):
    ...
    Args:
        ...
        cosFace_loss_func (nn.Module): CosFace module
    ...
    # optimizer = optim.RMSprop(
        chain(cls_model.parameters(), cosFace_loss_func.parameters()),
        lr=params.lr,
        # weight_decay=0.05
        weight_decay=5e-4
    )
    ...
    def handle_sigterm(signum, frame):
        ...
        torch.save(
            {"cls_model":cls_model.state_dict(), params.loss_fuc_type:cosFace_loss_func.state_dict()}, 
    ...
    for epoch in range(params.total_epochs):
        ...
        for step, (images, labels) in enumerate(iter_object):
            ...
            logits = cosFace_loss_func(feats, labels)
    ...
    # end all epoch
    return cls_model, cosFace_loss_func


@torch.no_grad()
def validate_procedure(
    cls_model:nn.Module, 
    CosFace_loss_module:nn.Module,
    ...
    Args:
        ...
        CosFace_loss_module (nn.Module): CosFace module, which includes a fc layer
...
@torch.no_grad()
def get_feats_labels(cls_model:nn.Module, dataloader:DataLoader, params:TrainCosFaceParams):
...
@torch.no_grad()
def get_cross_entropy_loss(cls_model:nn.Module, CosFace_loss_module:nn.Module, dataloader:DataLoader, params:TrainCosFaceParams):
    # setup the model on eval mode
    cls_model.eval()
    CosFace_loss_module.eval()
    ...
    for (images, labels) in iter_object:
        ...
        feats = cls_model(images)
        logits = CosFace_loss_module(feats, labels)   # [b, feats_dim] * [feats_dim, n_class] -> [b, n_class]
...
def trainCosFaceMain():
    ...
    cls_model, cosFace_loss_func = defineModel(
        params.dataset_name, params.backbone_type, params.loss_fuc_type, params.n_class)
    print(type(cls_model))
    print(type(cosFace_loss_func))

    # ------------------------------
    # -- train model
    # ------------------------------
    print("=== train model ===")
    cls_model, cosFace_loss_func = train_procedure(
        params,
        cls_model, cosFace_loss_func,
        train_dataloader, val_dataloader, 
        # test_dataloader
    ...
    cosFace_loss_func.load_state_dict(archive[params.loss_fuc_type])
    # setup mode
    cls_model.eval()
    cosFace_loss_func.eval()
