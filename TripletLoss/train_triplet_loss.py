...
from TripletLoss.similarity.sampler import PKSampler
from TripletLoss.similarity.triplet_loss import TripletMarginLoss
from global_utils import identification_procedure, prepareEnv, verification_procedure
from load_data.get_dataloader import getDataloader, getDataset
...
class TrainTripletLossParams(ParamsParent):
    ...
    labels_per_batch_p = 8
    samples_per_label_k = 16
    batch_size = labels_per_batch_p * samples_per_label_k
    lr = 5e-5
    ...
    # ckp_time_stamp = "2024-10-16_11-30"   # exp9   cifar100   resnet18   triplet
    ckp_time_stamp = "2024-10-16_11-31"   # exp10   cifar10   resnet18   triplet
    ...
    # nohup python -u main.py > ./TripletLoss/log/2024-10-16_11-30.txt 2>&1 &
    # exp9      1976631
    # nohup python -u main.py > ./TripletLoss/log/2024-10-16_11-31.txt 2>&1 &
    # exp10      1982912
...
def train_procedure(
    ...
    Args:
        params (TrainTripletLossParams): all the parameters
    ...
    # --  setup loss function
    # ------------------------------
    triplet_loss_func = TripletMarginLoss(margin=params.margin)
    ...
    for epoch in range(params.total_epochs):
        ...
        for step, (images, labels) in enumerate(iter_object):
            ...
            # 计算损失值
            batch_loss, _ = triplet_loss_func(feats, labels)
...
def validate_procedure(
    ...
    Args:
        ...
        params (TrainTripletLossParams): External parameters
    """
...
@torch.no_grad()
def get_feats_labels(cls_model:nn.Module, dataloader:DataLoader, params:TrainTripletLossParams):
...
@torch.no_grad()
def get_cross_entropy_loss(cls_model:nn.Module, dataloader:DataLoader, params:TrainTripletLossParams):
    # setup the model on eval mode
    cls_model.eval()
...
def trainTripletLossMain():
    ...
    # all_train_labels = train_dataset.targets.tolist()   # 所有的标签
    all_train_labels = [train_dataset[i][1].item() for i in range(len(train_dataset))]
    train_dataloader = DataLoader(
        train_dataset, batch_size=params.batch_size, 
        sampler=PKSampler(all_train_labels, params.labels_per_batch_p, params.samples_per_label_k),
        num_workers=1
    )
