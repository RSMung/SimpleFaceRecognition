import os
# https://github.com/pytorch/vision/blob/main/references/similarity/train.py
import torch
...
def main(args):
    ...
    if args.use_deterministic_algorithms:
        # 禁用 CuDNN 的基准模式
        # 这意味着 CuDNN 不会根据输入的大小和网络的配置自动寻找最佳的卷积算法
        # 从而确保每次运行的结果是一致的，避免非确定性结果。
        torch.backends.cudnn.benchmark = False
        # 启用 PyTorch 的确定性算法，
        # 这样可以确保每次运行代码时结果都是相同的，适合需要重现实验结果的场景
    ...
    targets = train_dataset.targets.tolist()   # 所有的标签
    # print(type(targets))   # list
    # print(targets)
    # print(len(targets))   # 60000
...
def parse_args():
    ...
    # parser.add_argument(
    #     "--dataset-dir", 
    #     default="/home/RSMung/data/fmnist/", 
    #     type=str, help="FashionMNIST dataset directory path"
    # )
    parser.add_argument(
        "--dataset-dir", 
        default="D:\ML_data", 
